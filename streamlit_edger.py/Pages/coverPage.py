...
def create_form(form_fields_list, form_fields_data):
    ...
    for cp_field_name, field_type, field_options in form_fields_list:
        # Skip manager fields for the cover page
        if cp_field_name in [field[0] for field in manager_fields_group]:
            continue

        if cp_field_name not in displayed_fields:
            if cp_field_name in form_fields_data:
                values = form_fields_data[cp_field_name]
                if isinstance(values, list):
                    for i, value in enumerate(values):
                        key = f"{cp_field_name}_{i}"
                        if field_type == "text_input":
                            st.text_input(
                                cp_field_name, value=value, disabled=True, key=key
                            )
                else:
                    st.text_input(cp_field_name, value=values, disabled=True)
            else:
                st.text_input(
                    cp_field_name, value="", disabled=True, key=f"{cp_field_name}"
                )
                st.warning(f"Field '{cp_field_name}' not found in the data.")
            displayed_fields.add(cp_field_name)


def form_Selection(form_fields_list, form_fields_data):
    st.markdown(
        "<h1 style='text-align: center;'>COVER PAGE </h1>", unsafe_allow_html=True
    )
    ...
    with st.form("13F_Form_Cover_Page"):
        ...
        if filer_cik:
            ...
            st.success("Success")
        cols = st.columns(2)
        if cols[0].form_submit_button("FILER INFORMATION"):
            switch_page("filerInfo")
            save_session_state(session_id, st.session_state)
        if cols[1].form_submit_button("COVER PAGE (MANAGER)"):
            switch_page("Manager")
            save_session_state(session_id, st.session_state)
...
cover_page_form_fields_list = [
    ...
    ("CP.2.c SecuritiesHoldingsConfidentialStatus", "text_input", None),
